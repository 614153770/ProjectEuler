...
*/
    ...
    this.debounced = function(name, fn, tm) {
      ...
      if (!dd) {
        setTimeout(function() {
          var fn;
          //如果外部已经调用remove销毁实例那么跳过处理逻辑
          if(self!==null && self.deleting===false){
              fn = self.delaylist[name];
...
//	      timer=setTimeout(function(){
	    	  var wpos,
                pos;
            if(self===null || self.deleting===true){
                return;
            }
              wpos = self.getOffset();
              pos = {
	          ...
	          if (!self.railslocked) self.rail.css({
	            top: pos.top+2,
	            left: pos.left-3,
	            height: ((len) ? len.h : self.win.innerHeight()) - (self.opt.railpadding.top + self.opt.railpadding.bottom) -4
	          });
...
//          border: self.opt.cursorborder,
        ...
        rail.addClass('scorll-bar scorll-bar-y');
...
*/
    ...
    this.showCursor = function(py, px) {
      ...
      self.cursor.css({
        ...
        top: self.scroll.y===0 ? self.scroll.y : self.scroll.y-5
      });
      if (self.cursorh) {
