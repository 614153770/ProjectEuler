 ...
 * @module ui-abstract
 */
define('ui-abstract', function(require, exports, moudles) {
    "require:nomunge,exports:nomunge,moudles:nomunge";
    var $=jQuery;
    /**
     * 抽象组件，此组件是所有组件的基类，不能直接实例化它!
     * @namespace ae.aeAbstract
     */
    $.aeWidget("ae.aeAbstract", {
        /**
         * 构造组件
         * @name ae.aeAbstract#_create
         * @function
         * @protected
         */
        _create:function(){
            $.error("An abstract class cannot be instantiated.");
        },
        /**
         * 显示组件
         * @name ae.aeAbstract#visible
         * @function
         */
        visible:function(){
        ...
        /**
         * 插入校验错误提示
         * @name ae.aeAbstract#insertErrorElement
         * @function
         */
        insertErrorElement:function(element,errorElement){
            errorElement.insertAfter(element.parent());
        },
        /**
         * 组件属性绑定
         * @param  {Object} element 组件初始dom节点
         * @param  {Object} $obj    新创建的jq节点对象
         * @return {void}
         */
        _bindAttributes:function($obj){
          var props={};
          $.each(this.element[0].attributes,function(key,value){
            //"data-ae-"前缀作为组件内部扩展属性名称空间使用
            if(!(value.name === 'id' || value.name === 'datafield' || value.name === 'class' || value.name.indexOf("data-ae-")===0)){
...
var aeDialog = $.aeWidget( "ae.aeDialog", {
    ...
    options: {
        ...
        width: 300,
        showButton:false,
        showCustomButton:false,//自定义按钮开关 与showButton参数互斥
        customButton:[
            {
                "text":"confirm",
    ...
    _create: function() {
        ...
        if(this.options.showButton){
            this._createButton();
        }else if(this.options.showCustomButton){
...
//        this._createButtonPane();
    ...
    close: function( event ) {
      ...
      //  this._destroyOverlay();
        ...
        //出现弹窗时禁用主页面滚动条，开启蒙层滚动条，关闭弹窗时删除蒙层滚动条，恢复主页面滚动条
...
//        });
    ...
    _createButton:function(){
	    ...
	    if(!this.options.showButton&&!this.options.showCustomButton){
	    	this.modalFooter.hide();
	    }
    },

    _createCustomButton:function(){
    	var self = this,
            btnClass = "btn-default",
            i=0;
	    this.modalFooter = $("<div>")
	        .addClass( "modal-footer modal-footer-user-defined" )
	        .appendTo( this.modalContent );

        for(i=this.options.customButton.length-1;i>=0;i--){
            var buttonProp = this.options.customButton[i];
            if(buttonProp.text){
                btnClass = buttonProp.isPrimary?"btn-primary":"btn-default";
                var buttonItem = $('<button class="btn '+btnClass+'" type="button">'+buttonProp.text+'</button>').appendTo( this.modalFooter );
            }
            if(buttonProp.onClick){
                self._on( buttonItem, {click: window[buttonProp.onClick]});
            }
        }


	    if((!this.options.showButton&&!this.options.showCustomButton)||this.options.customButton.length===0){
	    	this.modalFooter.hide();
	    }
    },
...
//            containment: "document",
            ...
            stop: function( event, ui ) {
                var offset = that.modalContent.offset(),
                    left = offset.left - that.document.scrollLeft(),
                    top = offset.top - that.document.scrollTop();

                options.height = that.modalContent.height()-128;
                options.width = that.modalContent.width();
                options.position = {
                    my: "left top",
                    at: "left" + (left >= 0 ? "+" : "") + left + " " +
                    "top" + (top >= 0 ? "+" : "") + top,
...
define('ui-calendar', function (require, exports, moudles) {
     ...
     */
    $.aeWidget('ae.aeCalendar',  $.ae.aeAbstract ,{
        /**
         * 可选项
        ...
        /**
         * 插入校验错误提示
         * @name ae.aeCalendar#insertErrorElement
         * @function
         * @param {object} 错误提示元素节点
         */
        insertErrorElement:function(element,errorElement){
            errorElement.insertAfter(element.parent().parent());
        },
        /**
         * 设置控件选中的日期
        ...
        _create: function () {
            ...
            var obj = $("<input></input>");
            !!this._bindAttributes &&　this._bindAttributes(obj);
            ...
            this.element.addClass('form-control').wrap('<div class="input-group"></div>');
             if(this.options.showTime){
                this.element.after('<span class="input-group-btn"><button class="btn btn-default"><i class="md-calendar-clock icon-sm" style="font-size:18px;"></i></button></span>');
            }else{
                this.element.after('<span class="input-group-btn"><button class="btn btn-default"><i class="md-calendar-blank icon-sm" style="font-size:18px;"></i></button></span>');
            }
        },
        _init: function () {
            var self = this,
        ...
        _attachDatePickerEvents: function () {
            var self = this;
            this.widget.off().on('click', '.datepicker *', $.proxy(self._click, this));
        ...
        _compileFormat: function () {
            var self = this,
                ...
                components = [],
                mask = [],
                str = self.format === self.defaultFormat ? 'yyyy-MM-dd hh:mm:ss' : self.format ,
...
define('ui-combo', function (require, exports, moudles) {
     ...
     */
    $.aeWidget('ae.aeCombo',$.ae.aeAbstract, {
        /**
         * 可选项
        ...
        clear: function () {
            ...
            if (!self.options.multi && self.options.showClear) {
                if (valueEl.prevAll().is('.md-window-close')) {
                    valueEl.prevAll('.md-window-close').hide();
                }
            }
        },
        /**
        ...
        enable: function (value) {
            ...
            if (value === true || value === 'true') {
                ...
                if (!self.options.multi) {
                    ...
                    if (self.options.showClear) {
                        if (this.element.prevAll().is('.md-window-close') && (val !== '' && typeof (val) != 'undefined')) {
                            this.element.prevAll('.md-window-close').show();
                        }
                    }
                } else {
                    ...
                    if (labelInfo.length > 0) {
                        labelInfo.find('>i.md-window-close').on('click',this, function (event) {
                            var inst=event.data,
                                arr = [],
                                s = '',
                                idx,
                            ...
                            idx=$(this).parent().remove().attr("data-ae-index");
                            curIdxs=inst.element.attr("data-ae-index");
                            ...
                            if(arrIdxs.length>0){
                                ...
                                this.element.attr("data-ae-index",arrIdxs.join(","));
                            }
            ...
            if (value === false || value === 'false') {
                ...
                if (!self.options.multi) {
                    ...
                    if (self.options.showClear) {
                        if (this.element.prevAll().is('.md-window-close') && (val !== '' && typeof (val) != 'undefined')) {
                            this.element.prevAll('.md-window-close').hide();
                        }
                    }
                } else {
                    ...
                    labelInfo = valueEl.parent().find('>span.label-info');
                    if (labelInfo.length > 0) {
                        labelInfo.find('>i.md-window-close').off();
                    }
                }
            }
        },
        _init: function () {
            ...
            if (options.showClear && !options.multi) {
                ...
                $('<i class="md-window-close"></i>').prependTo(div).on('click',this, function (event) {
                    var oldValue=valueEl.val(),
                        inst=event.data;
                    inst.element.attr("data-ae-index","");
                    options.value = '';
                    inputEl.val('');
                    valueEl.val('');
                    $(this).hide();
        ...
        _create: function () {
            ...
            this.element.parent().parent().addClass('v-middle');

            //zengjie add by 16/5/16 添加带label的下拉
            if(!options.optionLabel) {//添加label
                $('<i class="md-expand-more"></i>').insertBefore(this.element);
            }
            if (options.multi) {
            ...
            //zengjie add by 16/5/16 添加带label的下拉
            if(options.optionLabel){//添加label
                var _$wrap = valueEl.next('div.input-icon');
                this.optionLable=_$wrap;
                _$wrap.addClass('input-label');
                _$wrap.find('input.form-control').removeClass('form-control');
                this.element.before('<span class="text-muted optionLabel"  title="'+options.optionLabel+'">'+options.optionLabel+'</span>');
                _$wrap.append('<i class="md-expand-more"></i>');
            }
        ...
        _bindEvent: function () {
            ...
            input.on("focus",this,function (event) {
                var inst=event.data;
                inst._refeshEmptyText(emptyText);
                if (inst.options.multi && options.emptyText) {
                    var em = inst.element.parent().find('>em');
                    if (em.length > 0) {
                        em.remove();
                    }
                }
                if (options.onClick) {
                    inst._trigger("onClick", null, event);
                }
                inst._showDropList();
                ...
                if (inst.options.multi && options.emptyText) {
                    span = inst.element.parent().find('>span');
                    if (span.length <= 0) {
                        $('<em class="label-tip">' + inst.options.emptyText + '</em>').prependTo(inst.element.parent()).width(inst.element.parent().width() - 12);
                    }
                }
                $(document).off('keydown.aeCombo');
            });
            //为了特定需求,需要编辑下拉框(一般情况下不许使用)
            if(!options.editable){
            	input.on("keydown",this,function(event){
	                var inst=event.data,
	                    curKey = event.which,
                        source,
	                    hoverColor = '#EDF1F2',
	                    ...
	                    $(document).on('keydown.aeCombo',function(e){
                            if(curKey !== 9){
                                e.stopPropagation();
                                e.preventDefault();
                            }
	                    ...
	                    if(curKey === 40){
	                        if(inst._keyIndex === curDropListItems.size()-1){
	                            inst._keyIndex = -1;
	                            ...
	                            if(curDropList.hasClass('scorll-bar-box')){
	                                ...
	                                inst._scrollTop = 0 - curDropListItemHeight;
	                            }
	                            return false;
	                        }
	                        curDropListItems.eq(++inst._keyIndex).css('background-color',hoverColor).addClass('item-selected');
	                        if(curDropList.hasClass('scorll-bar-box')) {
	                            curDropList.getNiceScroll(0).doScrollTop(inst._scrollTop, 100);
	                            if (inst._scrollTop >= allScrollTop) {
	                                inst._scrollTop = allScrollTop + curDropListItemHeight;
	                            } else {
	                                inst._scrollTop += curDropListItemHeight;
	                            }
	                        }
	                    }else if(curKey === 38){
	                        if(inst._keyIndex === -1 || inst._keyIndex === 0){
	                            inst._keyIndex = curDropListItems.size() - 1;
	                            curDropListItems.eq(inst._keyIndex).css('background-color',hoverColor)
	                                .addClass('item-selected');
	                            if(curDropList.hasClass('scorll-bar-box')) {
	                        ...
	                        source = curDropList.children('li.item-selected').children('a');
	                        inst._backfill(source);
	                        if(options.multi){
	                        }else{
	                            inst.textInput.blur();
	                        }
            ...
            valueEl.parents('.input-icon').find('>i.md-expand-more').click(function () {
            ...
            //zengjie add by 16/5/16 用于optionLabel添加focus的样式
            if(this.optionLable){
                this.optionLable.off('focusin.aeCombo').on('focusin.aeCombo',function(e){
                    $(this).addClass('input-label--focus');

                }).off('focusout.aeCombo').on('focusout.aeCombo',function(e){
                    $(this).removeClass('input-label--focus');
                });
            }
        },
        _showDropList: function () {
            ...
            this._keyIndex = -1;
            $listRows.removeClass('current').find('>i.md-check').remove();
            if (nowValue !== undefined && nowValue !== '') {
                ...
                if (options.multi) {
                    ...
                    for (i = 0; i < selectedValues.length; i++) {
                        ...
                        if (index > -1) {
                            $(dropList.find('li').get(index)).addClass('current').prepend('<i class="md-check"></i>');
                        }
                    }
                    ...
                    if (index > -1) {
                        valuedItem = $(dropList.find('li').get(index)).addClass('current').prepend('<i class="md-check"></i>');
            ...
            if (ss && ss.popupType !== 'div') {
                var popup = $("#" + ss.elementId).offset();
                widgetTop = inputPos.top + span.find('input').outerHeight() - popup.top + 6;
                widgetLeft = inputPos.left - popup.left;
            } else {
                widgetTop = inputPos.top + span.find('input').outerHeight() + 6;
        ...
        _wrapText: function (text,index) {
            if (typeof this.options.optionField === 'string') {
                return '<li data-ae-index="'+index+'"><a title="' + text + '">' + text + '</a></li>';
            } else {
                return '<li data-ae-index="'+index+'"><a>' + text + '</a></li>';
            }
        },
        ...
        _backfill: function (source) {
            ...
            if (multi) {
                ...
                if (obj.find('>i.md-check').length > 0) {
                    obj.find('>i.md-check').remove();
                } else {
                    obj.prepend('<i class="md-check"></i>');
                }
            } else {
                $li=this.dropList.find(".current");
                $li.removeClass("current").find('>i.md-check').remove();
                $(source).parent().addClass('current').prepend('<i class="md-check"></i>');
            }
            ...
            if(multi===true){
                this.element.attr("data-ae-index",idxs.join(","));
            }else{
                this.element.attr("data-ae-index",(idxs[0] || ""));
            }
            ...
            if($.isArray(index)===true){
                if($.isArray(data)===true && index.length===data.length){
                    for(i=index.length-1;i>=0;i--){
                        $li=this.dropList.find('li[data-ae-index="'+index[i]+'"]');
                        if($li.length>0){
                ...
                $li=this.dropList.find('li[data-ae-index="'+index+'"]');
                if($li.length>0){
         ...
         */
        getIndex:function(){
            var s=this.element.attr("data-ae-index"),
                arr,
                i;
            if(s){
        ...
        _setValue: function (value) {
            ...
            if (options.multi) {
                ...
                for (var j = 0; j < inputText.length; j++) {
                    innerHtml += '<span class="label label-info label-as-badge" data-ae-index="'+idxs[j]+'" value="' + values[j] + '">' + inputText[j] + '<i class="md-window-close"></i></span>';
                }
                ...
                }).find('>i.md-window-close').on('click',this, function (event) {
                    var inst=event.data,
                        val = [],
                        objVal = '',
                        curIdxs,
                ...
                if (options.showClear && options.enable) {
                    if (valueEl.val() !== '' && typeof (valueEl.val()) != 'undefined') {
                        valueEl.prevAll('.md-window-close').show();
                    }
                }
...
define('ui-textarea',function (require, exports, moudles) {
	"require:nomunge,exports:nomunge,moudles:nomunge";
	 $.aeWidget('ae.aeTextarea', $.ae.aeAbstract, {
		 options:{
             /**
              * 是否禁用组件。如果禁用，则不可以输入，form提交时也将忽略这个输入框。
              * @type Boolean
			  ...
			  * @default 无限制
			  * @example
			  * emptyText : "please input your story"
			  */
			 emptyText:''
       ...
       !!this._bindAttributes &&　this._bindAttributes(obj);
		 ...
		 _init:function(){
			 ...
			 if(options.emptyText){
				 inputEl.attr('placeholder',options.emptyText);
			 }
...
define('ui-textfield',function (require, exports, moudles) {
	"require:nomunge,exports:nomunge,moudles:nomunge";
	 $.aeWidget('ae.aeTextfield', $.ae.aeAbstract,{
		 options:{
             /**
              * 是否禁用组件。如果禁用，则不可以输入，form提交时也将忽略这个输入框。
              * @type Boolean
		 ...
		 _init:function(){
		     ...
		     if(options.showClear && options.visible && options.enable){
	    		 ...
	    		 var inputClear = $('<i class="md-window-close"></i>').insertAfter(this.element).hide();
	    ...
	    */
	    visible : function(value){
	    	...
	    	if(value === true || value === 'true'){
	    		$ele.removeClass('hidden');
	    		this.options.visible = true;
	    		var inputClear = $('<i class="md-window-close"></i>').insertAfter($ele).hide();
	    	...
	    	if(value === false || value === 'false'){
	    		$ele.addClass('hidden');
	    		this.options.visible = false;
	    		if($ele.next().is('.md-window-close')){
	    			$ele.next().remove();
	    		}
	    	}
	    },
...
define('ui-textpopup',function (require, exports, moudles) {
	"require:nomunge,exports:nomunge,moudles:nomunge";
	 $.aeWidget('ae.aeTextpopup', $.ae.aeAbstract, {
		 options:{
       ...
       !!this._bindAttributes &&　this._bindAttributes(obj);
             ...
             $self.empty().hide();
             this.element.addClass('form-control').wrap('<div class="input-group"></div>');
             this.element.after('<span class="input-group-btn"><button class="btn btn-default"><i class="md-search icon-sm" style="font-size: 18px;"></i></button></span>');
		 },
		 _init:function(){
			 var self = this,
			 	$ele = this.element,
			 ...
			 if(opts.showClear && opts.enable){
				 $ele.wrap('<div class="input-icon right"></div>');
	    		 $('<i class="md-window-close icon-sm" style="font-size: 18px;"></i>').insertAfter($ele).hide();
				 $ele.next().click(function(){
					 self.clear();
				 });
		     }
     ...
     $.extend($.ae.aeTextpopup, {
		   closePopup:function(json,textField,valueField){
			   ...
			   if(json){
					...
					if(object && object.next().is('.md-window-close')){
...
define('ui-flip',function(require, exports, modules) {
     ...
     * @namespace ae.aeFlip
     */
	$.aeWidget('ae.aeFlip', $.ae.aeAbstract,{
		/**
         * 可选项
	    ...
	    */
		 ...
		 * */
		 ...
		 _create:function(){
             ...
             $self.empty().hide();
             this.element.addClass('form-control').wrap('<div class="input-group"></div>');
             this.element.after('<div class="input-group-btn-vertical"><button type="button" class="btn btn-default btn-icon" data-flip="up"><i class="md-arrow-drop-up "></i></button><button type="button" class="btn btn-default btn-icon" data-flip="down"><i class="md-arrow-drop-down "></i></button></div>');
			 this._addEvents();
		 },
		 _addEvents:function(){
...
define('ui-advcombo',function (require, exports, moudles) {
	...
	$.aeWidget('ae.aeAdvcombo', {
		options : /** @lends aeAdvcombo#*/ {
			/**
			 * 是否添加到body下面 有弹出框的时候添加到body下面可能会出现不能销毁的bug 当为false的时候 只添加到当前element下
			 * @type Boolean
			 * @default true
			 */
			isDropContentAppendToBody: true,
			/**
			 * 防止重复初始化
			 * @type Boolean
			 * @default false
		...
		_create : function(){
			...
			$ele.empty();
			if(!options.isDropContentAppendToBody){
				$('[data-adc-body-id="'+this._id+'"]').appendTo($ele);
			}
			...
			if(options.renderer){
				...
				$ele.prepend('<div class="input-icon right"><div style="width: 100%;position:absolute;background-color:#e8e8e8;z-index:100;opacity:.6;display:none;" data-role="adv-masker-'+ this._id +'"></div><div class="form-control fx-input" style="height:'+options.inputHeight+'px;"></div><i class="md-window-close" style="display:none"></i><i class="md-keyboard-arrow-down" style="line-height:'+options.inputHeight+'px;"></i></div>');
			}else{
				if(options.multi){
					$ele.prepend('<div class="input-icon right"><div style="width: 100%;position:absolute;background-color:#e8e8e8;z-index:100;opacity:.6;display:none;" data-role="adv-masker-'+ this._id +'"></div><i class="md-keyboard-arrow-down"></i><div class="form-control"><input value="Select please"></div></div>');
				}else{
					$ele.prepend('<div class="input-icon right"><div style="width: 100%;position:absolute;background-color:#e8e8e8;z-index:100;opacity:.6;display:none;" data-role="adv-masker-'+ this._id  +'"></div><i class="md-window-close" style="display:none"></i><i class="md-keyboard-arrow-down"></i><input class="form-control form-readonly" readonly placeholder="Select please" aeValidate="'+$ele.attr("aeValidate")+'"rules="'+$ele.attr("rules")+'"aeid="'+$ele.attr("aeid")+'"></div>');
				}
			}
		...
		_buildOptions : function(options,element){
			...
			initData = $.trim(element.attr("initData"));
			...
			$header.bind("click.header",function(e){
				if($body.is(":hidden")){
					$body.css({
						'overflow': 'hidden',
						'position':'absolute',
						'left': options.isDropContentAppendToBody?$header.offset().left : $header.position().left,
						'top': options.isDropContentAppendToBody?$header.offset().top + $header.outerHeight() : $header.position().top + $header.outerHeight(),
		...
		setValue : function(data){
			...
			if(options.renderer){
				...
				$input = $header.children('.fx-input:first');
				oldValue=$input[0].innerHTML;
				if(!options.multi){
				...
				if($input[0].innerHTML!==oldValue){
					this._trigger("onValueChange", null, null, $input[0].innerHTML, oldValue);
				}
			}else{
				oldValue=this.element.find('input.form-control').val();
				$input = $header.find('input');
				if(!options.multi){
					$input.val(data);
					options._dataMap = _data;
					if(data!==oldValue){
						this._trigger("onValueChange", null, null, data, oldValue);
					}
				}else{
					$input.val("");
					if($.inArray(_data, options._dataMap) === -1){
						$input.before('<span class="label label-info label-as-badge">'+data+'<i class="md-window-close"></i></span>');
					...
					if(oldValue!==""){
						this._trigger("onValueChange", null, null, "", oldValue);
					}
				}
			}
		},
		...
		_bindCloseEvent : function(span){
			span.find('i.md-window-close').bind("click.close",function(){
				span.remove();
			});
		},
		/**
		...
		/**
		 * 清除值
		 * @name ae.aeAdvcombo#clear
		 * @function
		 * @example
		 * $('#mycombo').aeAdvcombo('clear');
		 */
		clear : function(){
			var self = this ,
				valueEl = this.element.find('input.form-control');
			self.setValue('');
			valueEl.prevAll('.md-window-close').hide();
		},
    ...
    _backfill: function (source) {
        ...
        if (options.showClear && options.enable) {
            ...
            if (valueEl.val() !== '' && typeof (valueEl.val()) != 'undefined') {
                ...
                valueEl.prevAll('.md-window-close').show();
                valueEl.prevAll('.md-window-close').unbind("click").on("click",function(e){
                	e.stopPropagation();
    	            self.setValue('');
					valueEl.prevAll('.md-window-close').hide();
					valueEl.trigger("valuechange",valueEl);
...
//				$header.append('<div class="panel-btn"></div>');
			...
			if(options.iconClose){
				$icons.prepend('<a class="aePaneliconclose"><i class="md-window-close"></i></a>');
			...
			if(options.iconFold){
				$icons.prepend('<a class="aePaneliconfold"><i class="md-keyboard-arrow-down"></i></a>');
...
//				var $iconFold = $body.prev('div.panel-heading').find('>div.panel-btn i.icon-arrowlup-down');
				...
				$iconFold.children('i').removeClass("md-keyboard-arrow-down").addClass("md keyboard_arrow_up");
...
//				$iconFold.parent('a').html('<i class="icon-arrowlup-down"></i>');
				...
				var $iconFold = $body.prev('div.panel-heading').find('>div.panel-btn >a.aePaneliconfold');
				$iconFold.children('i').removeClass("md keyboard_arrow_up").addClass("md-keyboard-arrow-down");
			}
        ...
        _makeSketch: function () {
            ...
            $ele.find('>div.panel-heading >div.tab-listbox >ul li').each(function (index, item) {
                ...
                if (options.closable === true || ($.isArray(options.closable) && $.inArray(index, options.closable) !== -1)) {
                    $('<i class="md-window-close"></i>').insertAfter($a);
                }
            });
            //显示首先激活的页签
...
//		    options.tabMenu = element.attr("tabMenu")=='true' ? true : options.tabMenu;	//TODO：右键菜单
        ...
        _tool: function (tool) {
            ...
            if (options.theme === 4) {
                ...
                options._toolFn = '<div class="fn">' + options.tool() + '</div>' || '<div class="fn"><i class="md-settings"></i></div>';
...
//	        var items = this.items;
        ...
        _buildEvent: function () {
            var _self = this, $ele = this.element, options = this.options;
            var $iconClose = $ele.find('>div.panel-heading >div.tab-listbox >ul i.md-window-close');
            //close icon
...
//	        		_self._activate($(this).find('a').attr('tabId'));
            ...
            //scroller click
            $ele.find('>div.panel-heading >div.fn span').unbind('click.aetabs').bind('click.aetabs', function (e) {
                ...
                if ($(this).children('i').hasClass('md-keyboard-arrow-left')) {
                    if (visibleTab.length > 0 && visibleTab[0] - 1 > -1) {
                        _self._scrollTo(visibleTab[0] - 1);
                    }
                }
                if ($(this).children('i').hasClass('md-keyboard-arrow-right')) {
                ...
                //	_self._scroll(-dist, _self._scrollCbFn());
                //}
                if ($(this).children('i').hasClass('md-keyboard-arrow-down')) {
...
//	        $ele.find('>div.panel-heading >ul li').unbind('click.aetabs');
        ...
        _scrollTo: function (n) {
            ...
            if (this._checkScroller()) {
                ...
                var lDiff = parseFloat($heading.css('padding-left')) - lBorder;
                var rDiff = $iconRight.offset().left - rBorder;
                var outPosition=$heading.offset().left;
                if (lDiff >= 0) {
                    if (Math.abs(lDiff) < 3 && _self._getAlter(n + 1)) {//距离太小，不必要移动 转到下一个节点
                        _self._scrollTo(n + 1);
                    } else {
                        _self._scroll(lDiff+outPosition, _self._scrollCbFn());
                    }
                } else if (rDiff <= 0) {
        ...
        add: function (config) {
            ...
            if (_config.closable) {
                $('<i class="md-window-close"></i>').insertAfter($newLi.children('a'));
            }
...
//	        var scrOffset = distance > 0 ? $ul.prev().offset() : $ul.next().offset();
            ...
            var queuedFn = function (next) {
                ...
                var flag = false;
                //fix distance.
                var lfix = Math.min(Math.abs($firstLi.offset().left) +outPosition+ parseFloat($heading.css('padding-left')), distance);
                var lfix = Math.min(Math.abs($firstLi.offset().left) +outPosition+ parseFloat($heading.css('padding-left')), distance);
        ...
        _checkScroller: function () {
            ...
            if (totalWidth > $ul.parent().parent().innerWidth()) {
                if ($div.siblings().length === 0) {
                    ...
                    $('<span><i class="md-keyboard-arrow-left"></i></span>').appendTo($divFn);
                    $('<span><i class="md-keyboard-arrow-right"></i></span>').appendTo($divFn);
                    $('<span><i class="md-keyboard-arrow-down"></i></span>').appendTo($divFn);
                ...
                $div.siblings().remove();
                options.tools = false;
                $ul.animate({left: "0px"},'normal','swing');
                return false;
            }
        },
        /**
...
$.aeWidget("ae.aeGuide", {
	options: /**@lends aeGuide#*/{
         ...
         * @type Array
         * @default ["md person","md image","md edit","md calendar_clock"]
         * @example
         *
         */
		_icon : ["md-person","md-image","md-edit","md-calendar-clock","md-book","md-home","md-folder","md-save"],
		/**
         * 自定义icon，仅当 theme="1" 时生效
         * 如果用户未定义，则使用内置_icon，内置 8 种图标，循环展示
         * @type Array
         * @default [md-person,md-photo,md-edit,md-calendar-blank]
         * @example
         *
         * icon : [],
         */
...
//				$(item).children('a').html('<strong><i class="md-check"></i></strong><p>'+text+'<p>');
				$(item).children('a').html('<strong>'+(index+1)+'</strong><p>'+text+'<p>');
				options.icon.push("md-check");
			}
	...
	_stepsShow : function(index, type){
		...
		if(type === 'start'){
			...
			if(options.theme === 0){
				$li.prev('li').find('>a >strong').empty().html('<i class="md-check"></i>');
			}
...
//        		_self._calculateOffset();
        ...
        _renderHeader : function(data){
        	...
        	$ele.find('>ul:first >li').each(function(index,item){
        		...
        		if(mData && mData.length){
        			$(item).find('>a:first').append('<i class="md-keyboard-arrow-down"></i>');
		...
		_appendNodes : function(source, _hierarchy){
            ...
            var iClass = (_hierarchy === undefined) ? "md-keyboard-arrow-down" : "md-keyboard-arrow-right";
...
//                  left = offSet.left +  1;
            ...
            $($ele).children("ul.menu").children().each(function(index,li){
                if($(li).find("span:first").hasClass("menu-item-sep")){   //TODO hasClass直接导致IE9使用MaxWidth
 ...
 */
     ...
     * @最终展示页面代码如下
       <div class="menu-box">
			...
			<ul class="nav nav-pills nav-stacked">
				...
				<li><a><i class="icon-event"></i>Profile<i class="md-chevron-right pull-right"></i></a></li>
				<li><a><i class="icon-trending-down"></i>Messages<i class="md-chevron-right pull-right"></i></a></li>
			</ul>
		</div>
     *
     * @example
    ...
    $.aeWidget('ae.aeMenutree', {
        ...
        _create : function(){
			...
			if(options.search){
				$ele.prepend('<div class="input-icon right"><i class="md-search"></i><input type="text" class="form-control" value="keywords" /></div>');
			}
            options._initial = true;
        },
        ...
        _bindLiEvent : function(li){
            ...
            $(li).bind("click.menuItem",function(event){
                ...
                if(self_li.children('ul').length){
                	self_li.find('>a >i:last').addClass('md-keyboard-arrow-down').removeClass('md-chevron-right');
                	self_li.children('ul').slideDown();
                	self_li.siblings('li').has('>ul').find('>a >i:last').addClass('md-chevron-right').removeClass('md-keyboard-arrow-down');
		...
		_appendNodes : function(source, _hierarchy, unfold){
            ...
            $(source).each(function(index, item){
                if(item._children){
                    ...
                    item.icon ? childrenHtml.push("<i class=\""+item.icon+"\"></i>") : null;
                    childrenHtml.push(item[label]);
                    unfold ? childrenHtml.push("<i class=\"md-keyboard-arrow-down pull-right\"></i>") : childrenHtml.push("<i class=\"md-chevron-right pull-right\"></i>");
                    childrenHtml.push("</a>");
...
define('ui-validate', function(require, exports, moudles) {
	...
	$.validator = function(options, form) {
		...
		this.currentForm = form.find('input[aeValidate="true"],textarea[aeValidate="true"],*[aevalidate-ext="true"]');
		if (form.is('input[aeValidate="true"],textarea[aeValidate="true"],*[aevalidate-ext="true"]')) {
	...
	$.extend($.validator, {
		...
		defaults: {
			...
			onfocusin: function(element) {
				...
				if (!this.checkable(element) && (element.attr("aeid") in this.submitted || element.attr("id") in this.submitted || !this.optional(element))) {
					...
					element = this.clean(element);
						...
						if (this.errorList.length > 0) {
							if(this.settings.errorElement === 'label'){
								var errorLabel = $(element).parent().find('label.errorLabel');
								errorLabel.show().html(this.errorList[0].message);
							}else{
			...
			onkeyup: function(element) {
					...
					if (element.attr("id") in this.submitted || this.clean(element) == this.lastElement) {
						...
						var tooltip = $(document.body).find('div.tooltip');
						if(!this.element(element)&&this.errorList.length > 0){
			...
			onvaluechange: function(element) {
				if (element.attr("aeid") in this.submitted || element.attr("id") in this.submitted ||this.clean(element) == this.lastElement) {
					this.element(element);
				}
			}
		},
		...
		prototype: {
			...
			showErrors: function(errors) {
				...
				if (this.errorList && this.errorList.length > 0) {
					...
					$.each(this.errorList, function(index, obj) {
						if(_this.settings.errorElement === 'label'){
							var errorLabel = $(obj.element).parent().find('label.errorLabel');
							errorLabel.html(obj.message);
						}
						...
						//扩展组件未被.input-icon.input-group包裹时
						if(item.attr('aevalidate-ext')&&parent.length === 0){
							$(item[0].parentNode).addClass('has-error');
						}
					...
					$.each(item, function(index, obj) {
						if(_this.settings.errorElement === 'label'){
							var errorLabel = _this.currentElements.parent().find('label.errorLabel');
							errorLabel.hide();
						}else{
							var toolTip = $(document.body).find('div.tooltip');
							if (toolTip.length > 0) {
						...
						if (parent.length === 0) {
							...
							//扩展组件未被.input-icon.input-group包裹时
						if(item.attr('aevalidate-ext')&&parent.length === 0){
							$(item[0].parentNode).removeClass('has-error');
						}
			...
			defaultShowErrors: function() {
				var content = [];
				if(this.settings.errorElement === 'label'){
					content.push('<label class="errorLabel" style="color:red"></label>');
					if(this.currentElements.parent().find('label.errorLabel').length<=0){
						$(content.join('')).insertAfter(this.currentElements);
					}
				}else{
			...
			check: function(element) {
				...
				for (var method in rules) {
					...
					try {
						var result;
						if($(element).attr('aevalidate-ext')){
								if(!!this.settings[method]){	
									//若传入了和组件类型名称相同的处理对象，将该对象作为处理值;
									//否则返回元素的html作为处理值
									 result = this.settings[method].call(element);
								}else{
									result = false;
								}	
						}else{
...
//;(function($){
    ...
    var CLASSES =  {
            ...
            hitarea: "hitarea",
            faFolder:"md-folder",
            faFolderOpen:"md-folder-open",
            iconPage:"md-insert-drive-file"
        };
    $.aeWidget("ae.aeTree", {
        ...
        _appendNodes: function(target, nodes, bNode, isDrop) {
            ...
            for(var i = 0, l = nodes.length; i < l; i++){
                ...
                if(self.options.showIcon){
                  if(node.aeChildren && node.aeChildren.length>0){
                	  if(node.expanded && (node.expanded === 'true'|| node.expanded == true)){
                          ht.push("<i class='md-folder-open'></i>");
                	  }else{
                          ht.push("<i class='md-folder'></i>");
                	  }
                  }else{
                      ht.push("<i class='md-insert-drive-file'></i>");
                  }
                }
...
//	                  }
	         ...
	         if(nodes.length>0 && pNode){
	        	...
	        	str.push(ht.join(""));
	          	str.push("</div>");
	          	obj.append(str.join(""));
	          	var folder = obj.find(">div.tree-folder-header >i.md-file-outline");
		        if(folder.length>0){
		        	folder.removeClass("md-file-outline").addClass("md-folder-open");
		        }
    ...
    $.aeWidget('ae.aeGrid', {
        options:/** @lends aeGrid#*/{
            ...
            */
             ...
             */
    		_initial : false,
    		treeIcons : {fold : 'md-folder', unfold : 'md-folder-open', file : 'md-chart-bar'},
    		//是否可编辑，默认false
            editable : false,
            ...
            icons: {
                paginationSwitchDown: 'md-keyboard-arrow-down',
                paginationSwitchUp: 'md-keyboard-arrow-up',
                refresh: 'md-refresh',
                toggle: 'md-view-list',
                columns: 'md-view-column'
            },
...
//            this.$grid.attr("aeType","aeGrid");
        ...
        _initHeader : function () {
            ...
            $.each(ops.columns, function (i, column) {
                ...
                if (!column.visible) {
                    //构建初始隐藏列
                    display_ = sprintf('display: %s;',"none");
...
//                style += sprintf('width: %s'+ unitWidth +'; ', column.width ? column.width : ops.DEFINE.colDefWidth);
                ...
                html.push('<th', ' field="'+column.field+'"', class_, sprintf(' style="%s"', halign + style + display_), '>');
            ...
            this.$grid.off('click', '.th-inner').on('click', '.th-inner', function (event) {
                if (self.options.sortable && $(this).hasClass("sortable")) {
                    var thIndex = $(this).parent().index();
                    var $th = self.$header.find('th:eq('+thIndex+')').children();

                    var field = $th.parent().attr('field');
                    if(field.indexOf("|") > 0){
                        field = field.replace(new RegExp(/\|/g),'.');
                    }
                    var tempData = [];
                    if($th.hasClass("sortable-desc")){  //升序
                        $th.removeClass("sortable-desc").addClass("sortable-rise");
                        var fieldArr = $.parseJSON($(this).attr("data-sort"));
                    }else if($th.hasClass("sortable-rise")){  //降序
                        $th.removeClass("sortable-rise").addClass("sortable-desc");
                        var fieldArr = $.parseJSON($(this).attr("data-sort")).reverse();
                    }else{  //初次排序，默认升序
                        $th.addClass("sortable-rise");
                        var fieldArr = [];
                        for(var i = 0; i < self.data.length; i++){
                            fieldArr.push($.aries.common.getDataByDatafield(self.data[i],field));
                        }
                        fieldArr.sort();
                        $th.attr("data-sort",JSON.stringify(fieldArr));
                    }
                    for(var j = 0; j < fieldArr.length; j++){
                        for(var i = 0; i < self.data.length; i++){
                            if(fieldArr[j] == $.aries.common.getDataByDatafield(self.data[i],field)){
                                tempData.push(self.data[i]);
                            }
                        }
                    }
                    //重新处理表格缓存数据
                    self.currentData = tempData;
                    self._reload(tempData,true);

                    //恢复其他列默认排序样式
                    $th.parent().siblings().find('div.sortable').removeClass("sortable-rise").removeClass("sortable-desc").attr("data-sort","");
                    //TODO 排序动作触发事件
                    //self.onSort(event);
                }
            });
...
//                this.$loading.css('top', cellHeight + 'px');
        ...
        _initBody : function (data, fixedScroll) {
            var self = this,
            	ops = this.options,
                html = [],
...
//            this._triggerEvent('pre-body', data);
            ...
            $.each(data,function(i, item){
                ...
                $.each(self.header.fields, function (j, field) {
                    ...
                    //构建隐藏列
                    if(column){
                        display_ = (column.visible == false ? "display:none;" : "");
                    }
                    style = sprintf('style="%s"', csses.concat(self.header.styles[j]).join('; ') + display_);
                    //TODO 是否需要？
...
//                        self.header.formatters[j], [value, item, i], value);
            ...
            //表格内容超出设定高度自动出现纵向滚动条
            var tbodyTempHeight = this.$grid.height() - 38;
            var tbodyCurrentHeight = this.$body.height();
            if(tbodyCurrentHeight > tbodyTempHeight){
                this.$body.addClass("scorll-bar-box");
        ...
        _refreshHeaderCheckBox:function(){
        	...
        	var header = this.$grid.find('thead');
        ...
        _resetView : function (params) {
            ...
            //为表格设置宽度，当内容超出就出现横向滚动条
            if (this.options.width) {
                var width = this.options.width;
                $tableContainer.css('width', width + 'px');
            }
...
//                    padding += cellHeight;
        ...
        _reload : function (data, cache) {
            var fixedScroll = false,
            	hasNoRecords = true;
            //cache=true 表示对缓存数据的处理，比如排序，故不改变服务本身返回数据
            if(!cache){
            ...
            !cache && this._initData(data);
            if(hasNoRecords){
            	this._initPagination();
                this._initBody(data, fixedScroll);
            }
...
//          this.initSearch();
        ...
        //可选项设置原则：
        //构造组件实例的时候，可选项可以全部以属性
        //的形式定义在元素上也可以通过js代码指定，
        //但这两种方式只能选择一种，不能混用！！！
        _buildOptions:function(ops,el){
			...
			attrValue=el.attr("singleRowClass");
			if(attrValue!==undefined){
				ops.striped=(el.attr("singleRowClass")=="false"?true:false);
			}else{
				ops.striped=!ops.singleRowClass;
			}
        	ops.dataField=el.attr("dataField")||ops.dataField;
			attrValue=el.attr("pagingSize");
			if(attrValue!==undefined){
				if($.isNumeric(attrValue)){
					ops.pagingSize=parseInt(attrValue,10);
				}
        	}else if($.isNumeric(ops.pagingSize)){
				ops.pagingSize=parseInt(ops.pagingSize,10);
			}
			attrValue=el.attr("showMsg");
			if(attrValue!==undefined){
				ops.showMsg=(el.attr("showMsg")=="false"?false:true);
			}else{
				ops.showMsg=!!ops.showMsg;
			}
            //表格属性，是否允许排序
            attrValue=el.attr("sortable");
			if(attrValue!==undefined){
            	ops.sortable=(el.attr("sortable")=="true"?true:false);
			}else{
				ops.sortable=!!ops.sortable;
			}

        	ops.datafield=el.attr("datafield")||ops.datafield;
			attrValue=el.attr("isPaging");
			if(attrValue!==undefined){
        		ops.isPaging=(el.attr("isPaging")=="true"?true:false);
			}else{
				ops.isPaging=!!ops.isPaging;
			}
			...
			attrValue=el.attr("editable");
			if(attrValue!==undefined){
        		ops.editable=(el.attr("editable")=="true"?true:false);
			}else{
				ops.editable=!!ops.editable;
			}
			attrValue=el.attr("doSelectEvent");
			if(attrValue!==undefined){
            	ops.doSelectEvent=(el.attr("doSelectEvent")=="true"?true:false);
			}else{
				ops.doSelectEvent=!!ops.doSelectEvent;
			}
			...
			attrValue=el.attr("showFooter");
			if(attrValue!==undefined){
				ops.showFooter=el.attr("showFooter")=="true"?true:false;
			}else{
				ops.showFooter=!!ops.showFooter;
			}
			attrValue=el.attr("theme");
			if(attrValue!==undefined){
				ops.theme=el.attr("theme") || "";
			}
			attrValue=el.attr("showBorder");
			if(attrValue!==undefined){
	        	ops.showBorder=(el.attr("showBorder")=="true"?true:false);
			}else{
				ops.showBorder=!!ops.showBorder;
			}
        	ops.startField=el.attr("startField") || ops.startField;
        	ops.endField=el.attr("endField") || ops.endField;
        	ops.titleRenderer=el.attr("titleRenderer") || ops.titleRenderer;

        	this._buildOptionsEvents(ops);

        	cols=el.children("[datafield]");
        	if(cols.length>0){
        		ops.columns=[];
        	...
        	}else if($.isArray(ops.columns)){ //如果是js设置则校验且矫正列头属性值
				columnsInfo=ops.columns;
				for(i=columnsInfo.length-1;i>=0;i--){
					ci=columnsInfo[i];
					ci.field=(ci.datafield || "");
					if(ci.field !== ""){
        				ci.field = ci.field.replace(new RegExp(/\./g),'|');
        			}
					ci.title=(ci.title || "");
					ci.width=(ci.width || "");
					ci.headAlign=(ci.headAlign || "center");
					ci.align=(ci.align || "left");
					ci.visible=((ci.visible===false)?false:true);
					ci.wrap=((ci.wrap===true)?true:false);
					ci.showTitle=((ci.showTitle===true)?true:false);
					ci.renderer=($.isFunction(ci.renderer)?ci.renderer:undefined);
					ci.sortable=((ci.sortable===true)?true:false);
					ci.order=((ci.order in {"asc":1,"desc":1})?ci.order:"asc");
					if($.isPlainObject(ci.editor)===false){
						ci.editor={};
					}
					ci.editor.editable=((ci.editable===false)?false:true);
					ci.editor.type=((ci.editType in {"text":1,"select":1,"date":1,"popup":1})?ci.editType:"text");
					if($.isPlainObject(ci.editor.options)===false){
						ci.editor.options={};
					}
					if(ci.editor.type==="select"){
        				ci.editor.options.emptytext=(ci.emptyText || "");
        				ci.editor.options.dataSource=(ci.dataSource || "");
        			}else if(ci.editor.type==="date"){
        				ci.editor.options.dataField=(ci.dataField || "");
        				ci.editor.options.dateFormat=(ci.dateFormat || "");
        				ci.editor.options.showTime=ci.showTime;
        			}else if(ci.editor.type==="popup"){
        				ci.editor.options.popupType=ci.popupType;
        				ci.editor.options.popupSource=ci.popupSource;
        				ci.editor.options.popupTitle=ci.popupTitle;
        				ci.editor.options.popupWidth=ci.popupWidth;
        				ci.editor.options.popupHeight=ci.popupHeight;
        			}
				}
			}
        },
        ...
        _filterHeader : function(){
            var self = this, ops = this.options, $header = this.$header; //$header 为隐藏表头
            $.each(ops.columns,function(i, item){
                var $th = $header.find('[field="'+item.field+'"]');
                var style = $th.attr("style");
                var stop = style.indexOf("display");
                if(item.visible){
                    if(stop != -1){
                        style = style.substring(0,stop);
                    }
                }else{
                    if(stop == -1){
        ...
        /**
         * 获取表格所有列字段信息
         * @name aeGrid#getItems
         * @function
         * @returns Array [{"index":0,"title":"custName","visible":true}]
         *  index : 字段索引（唯一），从0开始计数；
         *  title : 字段显示名称
         *  visible : 字段状态，true表示显示，false表示隐藏
         * @example
         * //获取当前表格所有列字段信息
         * var listArr = $('.selector').aeGrid('getItems');
         */
        getItems : function(){
            var headerItems = [];
            var columns = this.options.columns;
            $.each(columns,function(i,item){
                headerItems.push({"index":i,"title":item.title,"visible":item.visible,"field":item.field});
            });
            return headerItems;
        },
        /**
         * 过滤表格字段，显示指定字段列，其他列隐藏
         * @name aeGrid#itemFilter
         * @function
         * @param arr Array,需要显示的字段列索引值数组
         * @example
         * //显示前三列，隐藏其他列
         * $('.selector').aeGrid('itemFilter',[0,1,2]);
         * //显示所有列
         * $('.selector').aeGrid('itemFilter');
         * $('.selector').aeGrid('itemFilter',[]);
         *
         */
        itemFilter : function(arr){
            //TODO 入参校验
            var columns = this.options.columns;
            var flag = false;
            if(!arr){
...
//;(function($){
    $.aries.messagebox = {
        ...
        _init: function (settings) {
             ...
             */
            if (s.type === 'alert') {
                msgBoxHtml.push('  <i class="md-error"></i>');
            }
            if (s.type === 'confirm') {
                msgBoxHtml.push('  <i class="md-info"></i>');
            }
            if (s.type === 'success') {
                msgBoxHtml.push('  <i class="md-check-circle"></i>');
            }
            if (s.type === 'error') {
                msgBoxHtml.push('  <i class="md-close-circle"></i>');
            }
...
//;(function($) {
    $.aeWidget('ae.aeSlider', {
        ...
        /**
         * 销毁组件。
         * @name aeSlider#destroy
         * @function
         * @example
         * $('#slider').aeSlider('destroy');
         */
        destroy : function(){
            var $ele = this.element;
            this._destroy($ele);
	 ...
	 $.aries.popup={
	    ...
	    _popupSettings:function(popupTitle,popupWidth,popupHeight,ss){
	        return {
				     ...
				     showCustomButton:ss.showCustomButton,
				 	 customButton:ss.customButton
	         };
		},
				    ...
				    if(popupParam){
	      		 	...
	      		 	$.ajax(popupSource, {
	    		 		...
	    		 		success: function(data, textStatus, jqXHR){
							...
							//TODO 考虑页面引入多个文件，存在多个body标签时会否有bug
							$(document.body).addClass("modal-open");
	  		        	    var obj=$.extend(true,{},$.extend({
	  					       elementId:"ae_popup_div_"+$.aries.popup.initCount
	  					    },ss));
	 ...
	 $.aeWidget('ae.aeTips', {
		 ...
		 _create:function(){
			...
			if(id){
			 ...
			 $ele.addClass('alert');
			 $('<i class="md-window-close pull-right"></i>').appendTo($ele);
		 },
		 _init:function(){
