...
//;(function($, undefined) {
	...
	$.aries.common = {
		includeHtml : function(id, url, callback, error, noTenant) {
			...
			if (url) {
				if(url.indexOf('http')!==0 && $.aries.config && $.aries.config.common.PAGE_REDIRECT_PREFIX){
					...
					if ($.aries.config.common.IS_USE_TENANT_PREFIX
							&& tenant && !noTenant) {
						if (url[0] == '/') {
							url = "/" + tenant + url;
						} else {
					...
					if (pagePrefix.substring(pagePrefix.length - 1,
							pagePrefix.length) == "/") {
						if (url[0] == '/') {
	    ...
	    setTenant : function (tenant,isTenantPrefix){
	    	...
	    	$.cookie($.aries.config.sec.SEC_AUTH_TENANT_KEY, tenant, {path : "/"});
	    	if(isTenantPrefix){
        ...
        _createDownloadForm : function(s) {
			...
			var roleType = $.cookie($.aries.config.sec.SEC_AUTH_ROLETYPE_KEY);
			roleType = (roleType === undefined ? '' : roleType);
			var zone = $.cookie($.aries.config.sec.SEC_AUTH_ZONE_KEY);
			zone = (zone === undefined ? '' : zone);
			//TODO
			//$.aries.config.common.IS_AJAX_URL_ENCODE == false ? params = buf.join("") : params = encodeURIComponent(buf.join(""));
			params = buf.join("");
			
			$("<input type='hidden' name='WEB_HUB_PARAMS' value='" + params + "' />" +
					"<input type='hidden' name='file' value='download' />" +
					"<input type='hidden' name='token' value='" + token + "' />" +
					"<input type='hidden' name='tenant' value='" + tenant + "' />" +
					"<input type='hidden' name='roleType' value='" + roleType + "' />" +
					"<input type='hidden' name='zone' value='" + zone + "' />").appendTo(form);
            $(form).appendTo('body');
            return form;
        },
...
//;(function($, undefined){
    $.aries.ajax = {
        ...
        request: function (url, params, callback, error, settings, header) {
            ...
            token = (token === undefined ? '' : token);
            var tenant = $.cookie($.aries.config.sec.SEC_AUTH_TENANT_KEY);
            tenant = (tenant === undefined ? '21' : tenant);
            var roleType = $.cookie($.aries.config.sec.SEC_AUTH_ROLETYPE_KEY);
            roleType = (roleType === undefined ? '' : roleType);
            var zone = $.cookie($.aries.config.sec.SEC_AUTH_ZONE_KEY);
            zone = (zone === undefined ? '' : zone);
            //如果开发模式下启用了租户模拟，未登录的时候使用模拟租户，方便开发调试。
            if($.aries.config.common.IS_DEVELOPE_MODE && tenant ==='' && $.aries.config.common.IS_SIMULATE_TENANT && $.aries.config.common.SIMULATE_TENANT_ID !==''){
                tenant = $.aries.config.common.SIMULATE_TENANT_ID;
            }
            buf.push("&token=");
            buf.push(token);
            buf.push("&tenant=");
                ...
                if (data) {
                    if (typeof data.hub_code != 'undefined') {
                        if (data.hub_code === '0' || data.hub_code == 0) {
                            $.aries.ajax._dealAuthLoginError(data.hub_value, data);
                            if (s.errorback && $.isFunction(s.errorback)) {
        ...
        get: function (url, params, callback, error, settings, code, value) {
            return $.aries.ajax.request(url, params, callback, error, settings, code, value);
        },
        post: function (url, params, callback, error, settings, code, value) {
            return $.aries.ajax.request(url, params, callback, error, $.extend({type: "POST", encoding: "UTF-8"}, settings), code, value);
        },
        _dealAuthLoginError: function (data, allData) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (value && value.code) {
                    switch (value.code) {
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_ERROR :
                            $.aries.fn.auth.login_error(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_TIMEOUT :
                            $.aries.fn.auth.login_timeout(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_TOO_LONG :
                            $.aries.fn.auth.login_too_long(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_ONLY_ONCE :
                            $.aries.fn.auth.login_only_once(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_OVERLOAD :
                            $.aries.fn.auth.login_overload(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_PWD_TOO_OLD :
                            $.aries.fn.auth.login_pwd_too_old(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FAILURE_PWD_HAS_SENSITIVE :
                            $.aries.fn.auth.login_pwd_has_sensitive(value.code, value.value);
                            break;
                        case $.aries.config.sec.AUTH_LOGIN_FIRST_CHANGE_PWD :
                            $.aries.fn.auth.login_first_change_pwd(value.code, value.value, allData);
                            break;
                        default :
                            break;
                    }
